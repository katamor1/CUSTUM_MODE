export function renderWorkflowBuilderStepOperationsScript(): string {
  return String.raw`
function makeStep(type) { const id = nextId(type === 'agent' ? 'analyze' : type === 'command' ? 'collect-context' : type === 'result' ? 'write-result' : 'confirm'); if (type === 'command') return { id: id, title: titleFromId(id), type: 'command', action: { provider: 'vscode.executeCommand', args: ['example.commandId'] }, resultKey: normalizeId(id) + 'Result' }; if (type === 'manual') return { id: id, title: titleFromId(id), type: 'manual', prompt: 'Confirm this step before continuing.', userAction: { message: 'Confirm this step before continuing.', completeLabel: '完了' } }; if (type === 'result') return { id: id, title: titleFromId(id), type: 'result', result: { source: 'state', stateKey: '', sinks: [{ type: 'file', path: '.bob/artifacts/' + id + '.md' }] } }; return { id: id, title: titleFromId(id), type: 'agent', prompt: 'Describe the task for Bob to perform.' }; }

import { CoreWorkflowDefinition, EngineStep, ResultSinkDefinition, ResultSourceDefinition } from "./model"
...
export function validateWorkflowText(options: ValidateWorkflowTextOptions): ValidateWorkflowResult {
  ...
  return { ok: parsed.ok && !diagnostics.some((item) => item.severity === "error"), workflow: parsed.ok ? parsed.workflow : undefined, diagnostics }
}

export function validateCoreWorkflow(workflow: CoreWorkflowDefinition, options: Omit<ValidateWorkflowTextOptions, "sourceId" | "text">): WorkflowDiagnostic[] {
  const diagnostics: WorkflowDiagnostic[] = []
  const filePath = options.filePath
  const stepIds = new Set<string>()
  const resultKeys = new Set<string>()

  if (workflow.todoRequired && workflow.todos.length === 0) diagnostics.push(error(filePath, "todoRequired is true but no todo items were found."))
  if (workflow.stepMessage === "step") for (const step of workflow.engineSteps) if (!step.prompt?.trim()) diagnostics.push(warning(filePath, `stepMessage is 'step' but step '${step.id}' has no prompt.`))
  ...
  for (const step of workflow.engineSteps) {
    ...
    if (step.maxResultBytes !== undefined && step.maxResultBytes <= 0) diagnostics.push(error(filePath, `Step '${step.id}' maxResultBytes must be greater than zero.`))
...
function validateCommandStep(step: Extract<EngineStep, { type: "command" }>, options: Omit<ValidateWorkflowTextOptions, "sourceId" | "text">, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  if (!step.action.provider.trim()) diagnostics.push(error(filePath, `Command step '${step.id}' action.provider is empty.`))
  if (options.availableActionProviders && !options.availableActionProviders.includes(step.action.provider)) diagnostics.push(error(filePath, `Command step '${step.id}' uses unsupported action provider '${step.action.provider}'.`))
}
...
function validateTodoStepMapping(workflow: CoreWorkflowDefinition, stepIds: Set<string>, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  ...
  for (const todo of workflow.todos) if (!stepIds.has(todo.id)) diagnostics.push(error(filePath, `Todo '${todo.id}' has no matching step.`))
  for (const step of workflow.engineSteps) if (!todoIds.has(step.id)) diagnostics.push(warning(filePath, `Step '${step.id}' has no matching Todo item.`))
}
...
function validateStateReferences(steps: EngineStep[], resultKeys: Set<string>, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  for (const step of steps) {
    for (const key of step.includeState ?? []) if (!resultKeys.has(key)) diagnostics.push(error(filePath, `Step '${step.id}' includeState references unknown resultKey '${key}'.`))
    if ((step.type === "agent" || step.type === "result") && step.result) validateResultSource(step.id, step.result, resultKeys, diagnostics, filePath)
  }
}

function validateResultSource(stepId: string, result: ResultSourceDefinition, resultKeys: Set<string>, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  if (result.source === "state" && !resultKeys.has(result.stateKey)) diagnostics.push(error(filePath, `Step '${stepId}' result references unknown stateKey '${result.stateKey}'.`))
...
function validateArtifacts(workflow: CoreWorkflowDefinition, stepIds: Set<string>, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  ...
  for (const artifact of workflow.artifacts) {
    ...
    if (artifact.producedBy && !stepIds.has(artifact.producedBy)) diagnostics.push(error(filePath, `Artifact '${artifact.id}' references unknown producedBy step '${artifact.producedBy}'.`))
  }
}
...
function validateInputs(workflow: CoreWorkflowDefinition, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  for (const [key, input] of Object.entries(workflow.inputs)) {
    if (input.type === "select" && (!input.options || input.options.length === 0)) diagnostics.push(error(filePath, `Input '${key}' is select but has no options.`))
    if (input.requiredWhen) {
      const referenced = input.requiredWhen.match(/^\s*(?:inputs\.)?([A-Za-z0-9_.-]+)\s*(?:==|!=)\s*.+$/)?.[1]
      if (!referenced) diagnostics.push(warning(filePath, `Input '${key}' has an unsupported requiredWhen expression: ${input.requiredWhen}`))
      else if (!workflow.inputs[referenced]) diagnostics.push(error(filePath, `Input '${key}' requiredWhen references unknown input '${referenced}'.`))
    }
  }
}

function validatePreflight(workflow: CoreWorkflowDefinition, options: Omit<ValidateWorkflowTextOptions, "sourceId" | "text">, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  if (!options.availablePreflightChecks) return
  for (const preflight of workflow.preflight) {
    for (const check of preflight.checks ?? []) if (!options.availablePreflightChecks.includes(check)) diagnostics.push(warning(filePath, `Preflight '${preflight.id}' references unknown check '${check}'.`))
  }
}
...
function validateGuardrails(workflow: CoreWorkflowDefinition, diagnostics: WorkflowDiagnostic[], filePath: string): void {
  ...
  for (const denied of workflow.guardrails.deniedCommands ?? []) if (allowed.has(denied)) diagnostics.push(error(filePath, `Guardrail command '${denied}' is both allowed and denied.`))
}
...
function diagnosticHint(message: string): string | undefined {
  if (message.includes("includeState references unknown resultKey")) return "Add resultKey to an earlier command or agent step, or remove the includeState entry."

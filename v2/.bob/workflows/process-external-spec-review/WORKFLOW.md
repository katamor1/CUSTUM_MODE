...
name: process-external-spec-review
title: 外部仕様レビュー
description: 外部仕様を要求・証跡と照合し、レビュー結果を工程記録へ保存します。
...
artifacts:
  ...
  - { id: reviewResult, producedBy: save-review-result, path: ".bob-process-runs/{{run.id}}/external-spec-review/review-result.yaml" }
...
steps:
  ...
  - id: draft-review-result
    ...
    prompt: |
      外部仕様レビューとして `process-review-result/v1` YAML だけを返してください。workflowName は process-external-spec-review です。
      要求充足、画面/API 入出力、例外時の振る舞い、受入条件、ユーザー影響を checklist 化し、証跡 id だけを evidenceRefs に入れます。
      Bazaar 調査では `bzr --no-aliases <command>` を必須にしてください。
  - id: save-review-result
    title: 外部仕様レビュー結果を保存
    type: result
    result: { source: state, stateKey: reviewResultYaml, sinks: [{ type: file, path: ".bob-process-runs/{{run.id}}/external-spec-review/review-result.yaml", encoding: utf8 }] }
  - id: validate-review-result
    title: 外部仕様レビュー結果を検証
    type: command
    prompt: 工程レビュー結果を process-review-result/v1 と evidence index に照らして検証してください。
    action: { provider: vscode.executeCommand, args: [bobProcess.validateReviewResult, { reviewResultPath: ".bob-process-runs/{{run.id}}/external-spec-review/review-result.yaml", evidenceIndexPath: ".bob-process-runs/{{run.id}}/evidence-index.json" }] }
    resultKey: reviewValidation
    sendResult: true
    completeOnSuccess: true
  - id: human-gate
    title: 外部仕様レビューを人間が確認
    type: manual
    prompt: 工程の成果物、検証結果、レビュー結果を人間が確認し、記録へ進むか差し戻すかを判断してください。
    approval: { resultKey: humanGate, approveLabel: 記録へ進む, rejectLabel: 差し戻す, message: 外部仕様レビューの指摘と証跡を確認してください。 }
  - id: write-process-record
    ...
    action:
      ...
      args:
        ...
        - record: { schemaVersion: bob-process-record/v1, campaignId: "{{json state.processInput.input.campaignId}}", runId: "{{run.id}}", workflowName: process-external-spec-review, phase: external_spec, status: completed, inputPath: "{{inputs.processInputPath}}", artifactRoot: ".bob-process-runs/{{run.id}}", evidenceIndexPath: ".bob-process-runs/{{run.id}}/evidence-index.json", reviewResultPath: ".bob-process-runs/{{run.id}}/external-spec-review/review-result.yaml", humanGate: { required: true, status: "{{json state.humanGate.decision}}" } }

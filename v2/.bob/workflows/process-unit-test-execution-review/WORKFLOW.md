...
name: process-unit-test-execution-review
title: 単体テスト実行レビュー
description: 単体テスト結果とログを確認し、実行レビュー結果を工程記録へ保存します。
...
artifacts:
  ...
  - { id: reviewResult, producedBy: save-review-result, path: ".bob-process-runs/{{run.id}}/unit-test-execution/review-result.yaml" }
...
steps:
  ...
  - id: draft-review-result
    title: 単体テスト実行レビュー結果を作成
    type: agent
    includeState: [processInput, evidenceIndex]
    stateRequired: true
    resultKey: reviewResultYaml
    prompt: |
      単体テスト実行レビューとして `process-review-result/v1` YAML だけを返してください。workflowName は process-unit-test-execution-review です。
      実行環境、成功/失敗、未実行、ログ根拠、再実行条件を checklist 化し、evidenceRefs は evidence-index の id だけを使います。
  - id: save-review-result
    title: 単体テスト実行結果を保存
    type: result
    result: { source: state, stateKey: reviewResultYaml, sinks: [{ type: file, path: ".bob-process-runs/{{run.id}}/unit-test-execution/review-result.yaml", encoding: utf8 }] }
  - id: validate-review-result
    title: 単体テスト実行結果を検証
    type: command
    prompt: 工程レビュー結果を process-review-result/v1 と evidence index に照らして検証してください。
    action: { provider: vscode.executeCommand, args: [bobProcess.validateReviewResult, { reviewResultPath: ".bob-process-runs/{{run.id}}/unit-test-execution/review-result.yaml", evidenceIndexPath: ".bob-process-runs/{{run.id}}/evidence-index.json" }] }
    resultKey: reviewValidation
    sendResult: true
    completeOnSuccess: true
  - id: human-gate
    title: 単体テスト実行結果を人間が確認
    type: manual
    prompt: 工程の成果物、検証結果、レビュー結果を人間が確認し、記録へ進むか差し戻すかを判断してください。
    approval: { resultKey: humanGate, approveLabel: 記録へ進む, rejectLabel: 差し戻す, message: テスト結果、ログ、未実行項目、再実行条件を確認してください。 }
  - id: write-process-record
    ...
    action:
      ...
      args:
        ...
        - record: { schemaVersion: bob-process-record/v1, campaignId: "{{json state.processInput.input.campaignId}}", runId: "{{run.id}}", workflowName: process-unit-test-execution-review, phase: unit_test, status: completed, inputPath: "{{inputs.processInputPath}}", artifactRoot: ".bob-process-runs/{{run.id}}", evidenceIndexPath: ".bob-process-runs/{{run.id}}/evidence-index.json", reviewResultPath: ".bob-process-runs/{{run.id}}/unit-test-execution/review-result.yaml", humanGate: { required: true, status: "{{json state.humanGate.decision}}" } }

...
TypeScript、JavaScript、Python、C#、Java、Go、Rust、Shell、SQL、JSON、YAML、Markdown、text、unknown は本書の深掘り対象外であり、`genericCodeEvidenceAnalyzer` が diff hunk 単位の `SRC-*` evidence として扱う。
...
- C/C++ header / define-only 変更で関数範囲を確定できない場合も、汎用 fallback evidence で Bob が参照できる根拠を残す。

## 3. 現行 runtime で抽出するシンボル
